...
namespace GraphVisualizer
{
    public class DefaultGraphRenderer : IGraphRenderer
    {
        ...
        private void DrawNode(Rect nodeRect, Node node, bool selected)
        {
            ...
            if (node is AnimationClipPlayableNode clipPlayableNode)
            {
                formatedLabel = clipPlayableNode.NodeDisplayName();
            }
            else
            {
                formatedLabel = Regex.Replace(nodeTypeLegend.label, "(\\B[A-Z])", "\n$1"); // Split into multi-lines
            }  
        ...
        private static GUIStyle _s1;

        private static GUIStyle S1
        {
            get
            {
                if (_s1 == null)
                {
                    _s1 = new GUIStyle(GUI.skin.label)
                    {
                        alignment = TextAnchor.MiddleCenter,
        ...
        private static GUIStyle S2
        {
            get
            {
                if (_s2 == null)
                {
                    _s2 = new GUIStyle(GUI.skin.label)
                    {
                        alignment = TextAnchor.MiddleCenter,
        ...
        private static void DrawEdge(Vector2 a, Vector2 b, float weight)
        {
            ...
            var s = weight > 0 ? S1 : S2;
            Handles.BeginGUI();
            Handles.Label((points[0] + points[1])/2, $"{weight:F4}", s);
        }
    }
}
